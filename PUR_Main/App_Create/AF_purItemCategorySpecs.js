...
var script_purItemCategorySpecs = {
    ...
    validate_ItemCategoryID: function(sender) {
      ...
      this.validated_FK_PUR_ItemCategorySpecs_ItemCategoryID_main = true;
      this.validate_FK_PUR_ItemCategorySpecs_ItemCategoryID(sender,Prefix);
      },
    ...
    validate_FK_PUR_ItemCategorySpecs_ItemCategoryID: function(o,Prefix) {
      var value = o.id;
      var ItemCategoryID = $get(Prefix + 'ItemCategoryID');
      if(ItemCategoryID.value==''){
        if(this.validated_FK_PUR_ItemCategorySpecs_ItemCategoryID_main){
      ...
      value = value + ',' + ItemCategoryID.value ;
        ...
        PageMethods.validate_FK_PUR_ItemCategorySpecs_ItemCategoryID(value, this.validated_FK_PUR_ItemCategorySpecs_ItemCategoryID);
      },
    validated_FK_PUR_ItemCategorySpecs_ItemCategoryID_main: false,
    validated_FK_PUR_ItemCategorySpecs_ItemCategoryID: function(result) {
      var p = result.split('|');
      var o = $get(p[1]);
      if(script_purItemCategorySpecs.validated_FK_PUR_ItemCategorySpecs_ItemCategoryID_main){
    ...
    validated_FK_PUR_ItemCategorySpecs_SpecID: function(result) {
      ...
      if(script_purItemCategorySpecs.validated_FK_PUR_ItemCategorySpecs_SpecID_main){
        var o_d = $get(p[1]+'_Display');
        try{o_d.innerHTML = p[2];}catch(ex){}
        var x =JSON.parse(p[3]);
        $get('F_SpecName').value = x.SpecName;
        $get('F_IsFixed').checked = x.IsFixed;
        $get('F_IsDerived').checked = x.IsDerived;
        $get('F_ValidateValue').checked = x.ValidateValue;
        $get('F_UseValues').checked = x.UseValues;
        $get('F_AllowUserValue').checked = x.AllowUserValue;
        $get('DDLpurValueDataTypes').value = x.ValueDataTypeID;
        $get("F_IsRange").checked = x.IsRange;
      }
      o.style.backgroundImage  = 'none';
      if(p[0]=='1'){
        o.value='';
