...
Partial Class GF_purOrderCancel
  Inherits SIS.SYS.GridBase
  Protected Sub GVpurOrderCancel_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVpurOrderCancel.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim OrderNo As Int32 = GVpurOrderCancel.DataKeys(e.CommandArgument).Values("OrderNo")
        Dim OrderRev As Int32 = GVpurOrderCancel.DataKeys(e.CommandArgument).Values("OrderRev")
        Dim RedirectUrl As String = TBLpurOrderCancel.EditUrl & "?OrderNo=" & OrderNo & "&OrderRev=" & OrderRev
    ...
    If e.CommandName.ToLower = "initiatewf".ToLower Then
      Try
        Dim BuyerRemarks As String = CType(GVpurOrderCancel.Rows(e.CommandArgument).FindControl("F_BuyerRemarks"), TextBox).Text
        Dim OrderNo As Int32 = GVpurOrderCancel.DataKeys(e.CommandArgument).Values("OrderNo")
        Dim OrderRev As Int32 = GVpurOrderCancel.DataKeys(e.CommandArgument).Values("OrderRev")
        SIS.PUR.purOrderCancel.InitiateWF(OrderNo, OrderRev, BuyerRemarks)
        GVpurOrderCancel.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    End If
    If e.CommandName.ToLower = "rejectwf".ToLower Then
      Try
        ...
        Dim OrderNo As Int32 = GVpurOrderCancel.DataKeys(e.CommandArgument).Values("OrderNo")
        Dim OrderRev As Int32 = GVpurOrderCancel.DataKeys(e.CommandArgument).Values("OrderRev")
        SIS.PUR.purOrderCancel.RejectWF(OrderNo, OrderRev, BuyerRemarks)
        GVpurOrderCancel.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    End If
  End Sub
  Protected Sub GVpurOrderCancel_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVpurOrderCancel.Init
    DataClassName = "GpurOrderCancel"
    SetGridView = GVpurOrderCancel
  End Sub
  Protected Sub TBLpurOrderCancel_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles TBLpurOrderCancel.Init
    SetToolBar = TBLpurOrderCancel
  End Sub
  ...
  Private Sub cmdAction_Click(sender As Object, e As EventArgs) Handles cmdAction.Click
    ...
    Dim OrderRev As String = F_OrderRev.Text
    Dim ReceiptNo As String = F_ReceiptNo.Text
    Dim oDs As List(Of SIS.PUR.purOrderDetails) = SIS.PUR.purOrderDetails.UZ_purOrderDetailsSelectList(0, 9999, "", False, "", OrderRev, OrderNo)
    For Each r As SIS.PUR.purOrderDetails In oDs
      SIS.PUR.purOrderDetails.OrderLineToReceiptLine(OrderNo, r.OrderLine, OrderRev, r.Quantity, ReceiptNo)
    Next
