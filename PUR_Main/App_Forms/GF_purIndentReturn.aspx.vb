...
Partial Class GF_purIndentReturn
  Inherits SIS.SYS.GridBase
  Protected Sub GVpurIndentReturn_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVpurIndentReturn.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim IndentNo As Int32 = GVpurIndentReturn.DataKeys(e.CommandArgument).Values("IndentNo")
        Dim RedirectUrl As String = TBLpurIndentReturn.EditUrl & "?IndentNo=" & IndentNo
    ...
    If e.CommandName.ToLower = "initiatewf".ToLower Then
      Try
        Dim ApproverRemarks As String = CType(GVpurIndentReturn.Rows(e.CommandArgument).FindControl("F_ApproverRemarks"), TextBox).Text
        Dim IndentNo As Int32 = GVpurIndentReturn.DataKeys(e.CommandArgument).Values("IndentNo")
        SIS.PUR.purIndentReturn.InitiateWF(IndentNo, ApproverRemarks)
        GVpurIndentReturn.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    End If
    If e.CommandName.ToLower = "rejectwf".ToLower Then
      Try
        Dim BuyerRemarks As String = CType(GVpurIndentReturn.Rows(e.CommandArgument).FindControl("F_BuyerRemarks"), TextBox).Text
        Dim IndentNo As Int32 = GVpurIndentReturn.DataKeys(e.CommandArgument).Values("IndentNo")
        SIS.PUR.purIndentReturn.RejectWF(IndentNo, BuyerRemarks)
        GVpurIndentReturn.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    End If
  End Sub
  Protected Sub GVpurIndentReturn_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVpurIndentReturn.Init
    DataClassName = "GpurIndentReturn"
    SetGridView = GVpurIndentReturn
  End Sub
  Protected Sub TBLpurIndentReturn_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles TBLpurIndentReturn.Init
    SetToolBar = TBLpurIndentReturn
  End Sub
  ...
  Private Sub cmdAction_Click(sender As Object, e As EventArgs) Handles cmdAction.Click
    ...
    Dim IndentNo As String = F_IndentNo.Text
    Dim oDs As List(Of SIS.PUR.purIndentDetails) = SIS.PUR.purIndentDetails.UZ_purIndentDetailsSelectList(0, 9999, "", False, "", IndentNo)
    For Each r As SIS.PUR.purIndentDetails In oDs
      SIS.PUR.purIndentDetails.IndentLineToOrderLine(IndentNo, r.IndentLine, r.Quantity, OrderNo, 0)
    Next
