...
Partial Class GF_purIndentApproval
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/PUR_Main/App_Display/DF_purIndentApproval.aspx"
  ...
  End Sub
  Protected Sub GVpurIndentApproval_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVpurIndentApproval.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim IndentNo As Int32 = GVpurIndentApproval.DataKeys(e.CommandArgument).Values("IndentNo")  
        Dim RedirectUrl As String = TBLpurIndentApproval.EditUrl & "?IndentNo=" & IndentNo
        Response.Redirect(RedirectUrl)
    ...
    If e.CommandName.ToLower = "approvewf".ToLower Then
      Try
        Dim ApproverRemarks As String = CType(GVpurIndentApproval.Rows(e.CommandArgument).FindControl("F_ApproverRemarks"),TextBox).Text
        Dim IndentNo As Int32 = GVpurIndentApproval.DataKeys(e.CommandArgument).Values("IndentNo")  
        SIS.PUR.purIndentApproval.ApproveWF(IndentNo, ApproverRemarks)
        GVpurIndentApproval.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    End If
    If e.CommandName.ToLower = "rejectwf".ToLower Then
      Try
        Dim ApproverRemarks As String = CType(GVpurIndentApproval.Rows(e.CommandArgument).FindControl("F_ApproverRemarks"),TextBox).Text
        Dim IndentNo As Int32 = GVpurIndentApproval.DataKeys(e.CommandArgument).Values("IndentNo")  
        SIS.PUR.purIndentApproval.RejectWF(IndentNo, ApproverRemarks)
        GVpurIndentApproval.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    End If
  End Sub
  Protected Sub GVpurIndentApproval_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVpurIndentApproval.Init
    DataClassName = "GpurIndentApproval"
    SetGridView = GVpurIndentApproval
  End Sub
  Protected Sub TBLpurIndentApproval_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles TBLpurIndentApproval.Init
    SetToolBar = TBLpurIndentApproval
  End Sub
