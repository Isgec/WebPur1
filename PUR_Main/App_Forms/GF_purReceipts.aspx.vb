...
Partial Class GF_purReceipts
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/PUR_Main/App_Display/DF_purReceipts.aspx"
  Protected Sub Info_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs)
    Dim oBut As ImageButton = CType(sender, ImageButton)
    Dim aVal() As String = oBut.CommandArgument.ToString.Split(",".ToCharArray)
    Dim RedirectUrl As String = _InfoUrl  & "?ReceiptNo=" & aVal(0)
    Response.Redirect(RedirectUrl)
  End Sub
  Protected Sub GVpurReceipts_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVpurReceipts.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim ReceiptNo As Int32 = GVpurReceipts.DataKeys(e.CommandArgument).Values("ReceiptNo")  
        Dim RedirectUrl As String = TBLpurReceipts.EditUrl & "?ReceiptNo=" & ReceiptNo
        Response.Redirect(RedirectUrl)
    ...
    If e.CommandName.ToLower = "Deletewf".ToLower Then
      Try
        Dim ReceiptNo As Int32 = GVpurReceipts.DataKeys(e.CommandArgument).Values("ReceiptNo")  
        SIS.PUR.purReceipts.DeleteWF(ReceiptNo)
        GVpurReceipts.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    End If
    If e.CommandName.ToLower = "initiatewf".ToLower Then
      Try
        Dim ReceiptNo As Int32 = GVpurReceipts.DataKeys(e.CommandArgument).Values("ReceiptNo")  
        SIS.PUR.purReceipts.InitiateWF(ReceiptNo)
        GVpurReceipts.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    End If
  End Sub
  Protected Sub GVpurReceipts_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVpurReceipts.Init
    DataClassName = "GpurReceipts"
    SetGridView = GVpurReceipts
  End Sub
